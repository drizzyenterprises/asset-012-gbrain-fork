...
describe('extractEntityRefs', () => {
  ...
  // ─── Fix #2 Edit D: ops/ in DIR_PATTERN ──────────────────────────

  test('extracts ops/ wikilinks as qualified refs (Fix #2 Edit D)', () => {
  ...
  test('extracts ops/ markdown links (Fix #2 Edit D)', () => {
  ...
  test('extracts ops/ qualified wikilinks (Fix #2 Edit D)', () => {
...
describe('makeResolver — fallback chain', () => {
  ...
  // ─── Fix #2 Edit E: path-style inputs to resolveBasenameMatches ────

  test('resolveBasenameMatches: path-style input resolves via tail (Fix #2 Edit E)', async () => {
    const engine = makeFakeEngineWithSlugs([
  ...
  test('resolveBasenameMatches: path-style input with spaces in tail (Fix #2 Edit E)', async () => {
    const engine = makeFakeEngineWithSlugs([
