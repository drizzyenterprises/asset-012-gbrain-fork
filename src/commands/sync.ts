...
async function performSyncInner(engine: BrainEngine, opts: SyncOpts): Promise<SyncResult> {
  ...
  for (const path of unsyncableModified) {
    ...
    // estate (PRJ-2026-032 #86): same for pruned-dir. A path under a pruned
    // segment (`ops/`, `node_modules/`, dot-dirs) was NEVER sync-eligible,
    // so any page at that slug necessarily came from put_page — deleting it
    // when the file is edited orphans the page and sync can never bring it
    // back. Tripped in production by capture merge-writes into ops/** pages.
